...
class _PromotionsPageState extends ConsumerState<PromotionsPage> {
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(promotionViewModelProvider);
    ...
    return Scaffold(
      ...
      body: Container(
        ...
        child: state.when(
          data: (promotions) => RefreshIndicator(
            ...
            onRefresh: () async {
              ...
              _controllerCache.clear();
              ref.read(promotionViewModelProvider.notifier).refreshList();
            },
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                if (index < promotions.length) {
                  ...
                  final vm = ref.read(promotionViewModelProvider.notifier);
                  if (vm.hasMore) {
                    vm.loadMore();
                    return const Center(
                      child: Padding(
...
class _PromotionItemState extends State<PromotionItem> {
  ...
  void _initializeController() async {
    if (widget.promotion.mediaType == 'video') {
      final url = widget.promotion.file;
            ...
            ClipRRect(
              ...
              child: ConstrainedBox(
                constraints: BoxConstraints(
                  ...
                  maxHeight:
                      500, // Optional: Set a max height to prevent overly large media
                ),
                child: promo.mediaType == 'image'
                    ? CachedNetworkImage(
                        imageUrl: promo.file,
                        fit: BoxFit.contain,
